...
layer. `core/option-validation.js` has no runtime deps on any other layer.
...
The test suite under `test/` runs as native ESM through its own `test/package.json`, while the repository root intentionally stays CommonJS so the CLI entrypoint and config files keep their current behavior. Tests are grouped by concern under `test/cases/` — `test/cases/analysis/`, `test/cases/cli/`, `test/cases/formatters/`, `test/cases/mongo-driver/`, `test/cases/mongo-shell/`, `test/cases/parity/`, `test/cases/persistence/`, and `test/cases/plugins/` — with shared helpers under `test/helpers/` and static inputs under `test/fixtures/`. That Mocha lane also includes focused CLI tests that execute `bin/variety` and stub `mongosh` / `mongo`, MongoDB Node driver adapter tests that exercise the private programmatic boundary without shelling out, and parity tests that compare shell JSON output with direct driver-adapter results for supported analysis-only options. Analysis tests include deterministic property-based fuzzing with `fast-check` to exercise generated Mongo-like documents against core analyzer invariants.
